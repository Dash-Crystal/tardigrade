...
def load(filepath):
    ...
    for pi, player in enumerate(players):
        ...
        # Auto-detect rate: if reduced length == full length, rate=1 (128Hz)
        pos_rate = max(1, round(n / max(len(x_red), 1)))
        x = _interpolate(x_red, n, pos_rate)
        y = _interpolate(y_red, n, pos_rate)
        z = _interpolate(z_red, n, pos_rate)
